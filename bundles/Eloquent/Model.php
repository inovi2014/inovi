<?php
	...
	require_once LIBRARIES_PATH . DS . 'laravel' . DS . 'helpers.php';
	...
	abstract class Model
	{
		...
		public static function table($class)
		{
			...
			return Inflector::lower(Str::plural(static::model_name($class)));
		}
		...
		{
			$this->relating_key = (is_null($foreign_key)) ? Inflector::lower(static::model_name($this)).'_id' : $foreign_key;
		...
		public function belongs_to($model, $foreign_key = null)
		{
			...
			return static::query($model)->where(
				static::pk($model),
				'=',
		...
		public function has_and_belongs_to_many($model, $table = null, $foreign_key = null, $associated_key = null)
		{
			...
			$associated_key = (is_null($associated_key)) ? Inflector::lower(static::model_name($model)).'_id' : $associated_key;

			return static::query($model)
            ->select(
            	array(
            		static::table($model) . '.*',
            		$this->relating_table . '.' . $this->relating_key
            	)
            )
            ->join(
            	$this->relating_table,
		...
		{
			$models = array(
				Str::plural(static::model_name($model)),
				Str::plural(static::model_name($this))
			);
			...
			if ($this->exists) {
				$success = (
					$this->query->where(
						static::$primary_key,
						'=',
						$this->attributes[static::$primary_key]
					)->update($this->dirty) === 1
				);
			} else {
