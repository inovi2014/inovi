<?php
    ...
    class Model
    {
        ...
        {
            $this->_sort[$field] = ('desc' === Inflector::lower($dir)) ? -1 : 1;
            return $this;
        }
    	...
    	{
    		if ($this->exists && count($this->dirty) == 0) return true;
    		...
    		if ($this->exists) {
    			$success = $this->connection->update(
                    self::$collection,
                    array('_id' => $this->attributes['_id']),
                    array('$set' => $this->dirty),
                    $options
                );
    		} else {
    		...
    		($this->exists = true) && $this->dirty = array();
    	 ...
    	 * @return mixed
    	 */
    	public function one($model, $foreign_key = null)
    	{
    		$this->relating = __FUNCTION__;

    		return $this->oneOrMany($model, $foreign_key);
    	}
    	 ...
    	 * @return mixed
    	 */
    	public function many($model, $foreign_key = null)
    	{
    		$this->relating = __FUNCTION__;
    	 ...
    	 * the returned oneToOne relationship key would be manager_id.
    	 *
    	 * @param  string  $model
    	 * @param  string  $foreign_key
    	 * @return mixed
    	 */
    	public function oneToOne($model, $foreign_key = null)
    	{
    		$this->relating = __FUNCTION__;
    	 ...
    	 * with an appended _id. This is the same convention as one and many.
    	 *
    	 * @param  string  $model
    	 ...
    	 * @return mixed
    	 */
    	public function manyToMany($model, $relation_model, $foreign_key = null, $associated_key = null)
    	{
    		$this->relating = __FUNCTION__;
