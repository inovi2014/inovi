<?php
    namespace thinBundle;
    use \thinBundle\Exception\InvalidDataTypeException;
    ...
    abstract class Model
    {
        ...
        public function __construct( $data = array(), $mapFields = false, $exists = false)
        {
            ...
            if (is_null($this->_connection)) {
                ...
                $this->_connection = Mongodm::instance($config);
            }
        ...
        {
            if(!\Thin\Arrays::is($updateQuery)) throw new \Exception('$updateQuery should be an array');
            if(!\Thin\Arrays::is($options)) throw new \Exception('$options should be an array');
                ...
                foreach ($this->cleanData as $key => $value) {
                    if (isset($attrs[$key])
                        ...
                        if ($attrs[$key]['type'] == self::DATA_TYPE_REFERENCE &&
                            ...
                            if (!empty($data)) $this->cleanData[$key] = $data;
                        }
                    }
                }
            }
        ...
        public static function one($criteria = array(),$fields = array())
        {
            ...
            static::processCriteriaWithType($criteria);
            $result = static::connection()->find_one(static::$collection, $criteria, static::mapFields($fields));
        ...
        public static function find($criteria = array(), $sort = array(), $fields = array() , $limit = null , $skip = null)
        {
            ...
            $results =  static::connection()->find(static::$collection, $criteria, static::mapFields($fields));
            ...
            if ( ! empty($sort)) {
                $results->sort(static::mapFields($sort));
            }
        ...
        public static function distinct( $key , $criteria = array() )
        {
            ...
            return static::connection()->distinct($query);
        ...
        {
            static::processCriteriaWithType($criteria);
            $results =  static::connection()->find(static::$collection, $criteria);
            $results->limit(1);
        ...
        protected static function connection()
        {
            ...
            return Mongodm::instance($config);
        }
