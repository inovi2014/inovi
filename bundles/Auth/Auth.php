<?php
    ...
    class Auth
    {
        ...
        public function register(array $user)
        {
            ...
            $required = array('username', 'password', 'email', 'name', 'firstname');
            ...
            $role = $this->addRole(array('name' => $username));
        ...
        public function removeRole($role)
        {
            if (is_null($role) || !$role instanceof Container) {
                ...
                $role = $this->roleDb->where("name = " . $this->user->getUsername())->first(true);
                $db = jdb(Config::get('bundle.auth.database', 'auth'), Config::get('bundle.auth.table.userrole', 'userrole'));
                $userroles = $db->where('role_id = ' . $role->id())->exec(true);
                ...
                $rolepermissions = $db->where('role_id = ' . $role->id())->exec(true);
        ...
        {
            return $this->permissionDb->where("name = $name")->first(true);
        }
        ...
        public function login($username, $password)
        {
            ...
            if (is_null($user) || !$user instanceof Container) {
                $user = $this->userDb->where("username = $username")->where('password = ' . sha1($password))->first(true);
