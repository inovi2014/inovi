<?php
    ...
    class Crud
    {
        ...
        {
            $this->model    = $model;
            $config         = context('config')->load('crud');
        ...
        private function prepareFields()
        {
            ...
            foreach ($fields as $field) {
                ...
                if (false === $settings && true === $foreign && $field != $this->pk()) {
                    $this->config['fields'][$field] = array();
        ...
        {
            return true === $keys ? array_keys($this->model->map['fields']) : $this->model->map['fields'];
        }
        ...
        public function makeSearch()
        {
            ...
            if (!empty($where)) {
                ...
                $search .= '&nbsp;&nbsp;<a class="btn btn-warning" href="#" onclick="document.location.href = document.URL;"><i class="fa fa-trash-o icon-white"></i> Supprimer cette recherche</a>&nbsp;&nbsp;';
            }
        ...
        private function options($id, $plus = '')
        {
            ...
            $options .= '<td style="text-align: center;"><ul class="list-inline"><li><a rel="tooltip" title="afficher" class="btn btn-xs btn-info" href="/crud/static/read/table/' . $this->model->table . '/id/' . $id . '"><i class="fa fa-file"></i></a></li>';

            $options .= '<li><a rel="tooltip" title="dupliquer" class="btn btn-xs btn-warning" href="/crud/static/duplicate/table/' . $this->model->table . '/id/' . $id . '"><i class="fa fa-copy"></i></a></li>';

            $options .= '<li><a rel="tooltip" title="éditer" class="btn btn-xs btn-success" href="/crud/static/update/table/' . $this->model->table . '/id/' . $id . '"><i class="fa fa-edit"></i></a></li>';

            $options .= '<li><a rel="tooltip" title="supprimer" class="btn btn-xs btn-danger" href="#" onclick="if (confirm(\'Confirmez-vous la suppression de cet élément ?\')) document.location.href = \'/crud/static/delete/table/' . $this->model->table . '/id/' . $id . '\'; return false;"><i class="fa fa-trash-o"></i></a></li>' . $plus . '</ul></td>';
        ...
        private function readableQuery($query)
        {
            ...
            $query = repl(" '", ' <span style="color: red;">', $query);
