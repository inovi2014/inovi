<?php
    ...
    class Nma
    {
        ...
        {
            $html = '<h3 class="titleTable">' . $this->tableByName('nma_table')->find($id)->getName() . ' (' . $this->tableByName('nma_table')->find($id)->getNs() . ')</h3>';
        ...
        private function deleteRecordConfirm()
        {
            ...
            if (strlen($id) && strlen($table) && strlen($token)) {
                if ($token == sha1('deleteRecord' . $id . $table . date('dmY'))) {
                    $db     = $this->tableByName(
                        $this->tableByName('nma_table')->find($table)->getName(),
                        $this->tableByName('nma_table')->find($table)->getNs()
                    );
                    $obj    = $db->find($id);
                    if (!empty($obj)) {
                        $obj->delete();
        ...
        private function deleteStructureConfirm()
        {
            ...
            if (strlen($id) && strlen($token)) {
                if ($token == sha1('deleteStructure' . $id . date('dmY'))) {
                    $dbs = $this->tableByName('nma_structure');
                    $obj = $dbs->find($id);
                    if (!empty($obj)) {
                        $db = $this->tableByName(
                            $this->tableByName('nma_table')->find($obj->getTable())->getName(),
                            $this->tableByName('nma_table')->find($obj->getTable())->getNs()
                        );
                        $data = $db->fetch()->exec();
                        foreach ($data as $row) {
                            $f = $obj->field();
        ...
        private function emptyTable()
        {
            $id     = isAke($_REQUEST, 'id', null);
            if (strlen($id)) {
                $table  = $this->tableByName('nma_table')->find($id);
                if (!empty($table)) {
                    $key = sha1('emptyTable' . $id . date('dmY'));
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . '?action=emptyTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to empty the table &laquo; ' . $this->tableByName('nma_table')->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . '?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function removeTable()
        {
            ...
            if (strlen($id)) {
                ...
                if (!empty($table)) {
                    ...
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . '?action=removeTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to drop the table &laquo; ' . $this->tableByName('nma_table')->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . '?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        {
            $t      = $this->tableByName('nma_table')->find(request()->getId());
            if (empty($t)) {
            ...
            foreach ($res as $row) {
                ...
                foreach ($fields as $k => $field) {
                    ...
                    if (strstr($type, 'fk_')) {
                        ...
                        $fk = container()->nbm($fkTable)->find($value);
                        if (!empty($fk)) {
        ...
        {
            $dbTable    = container()->nbm('nma_table');
        ...
        private function importMap()
        {
            if (true === context()->isPost()) {
                ...
                $db         = container()->nbm($table->getName());
        ...
        private function editRecord()
        {
            ...
            $db         = container()->nbm($table->getName(), $table->getNs());
        ...
        {
            $dbt = $this->tableByName('nma_table');
            $t = $dbt->find($id);
            if (!empty($t)) {
    ...
    // container()->nbm('nma_auth')->create()->setLogin('gplusquellec')->setPassword(sha1('230266gp'))->save();

    $error = null;

    $dbAuth     = container()->nbm('nma_auth');
    $session    = session('nma');
        ...
        <div class="container">
            ...
            <h1 class="text-center">
                <span onclick="document.location.href = '<?php echo container()->getUrlsite(); ?>';" class="title">
                    <i class="fa fa-cogs fa-3x"></i>&nbsp;&nbsp;nodeMyAdmin
                </span>
            </h1>
