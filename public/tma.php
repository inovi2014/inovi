<?php
    ...
    class Tma
    {
        ...
        {
            session('tma')->setAuth(null);
            Router::redirect(container()->getUrlsite() . 'tma.php');
        }

        private function home()
        {
            return '<a href="' . container()->getUrlsite() . 'tma.php?action=addTable">
        ...
        {
            $html = '<h3 class="titleTable">' . with(container()->tbm('core_table'))->find($id)->getName() . ' (' . with(container()->tbm('core_table'))->find($id)->getNs() . ')</h3>';
            $first = $this->action != 'displayData'
            ? '<a href="' . container()->getUrlsite() . 'tma.php?action=displayData&amp;id=' . $id . '"><i class="fa fa-folder-open"></i> Display data</a>'
            : '<a href="' . container()->getUrlsite() . 'tma.php?action=table&amp;id=' . $id . '"><i class="fa fa-tasks"></i> Display structures</a>';
            $html .= '<p>
                ' . $first . ' |
                <a href="' . container()->getUrlsite() . 'tma.php?action=addRecord&amp;id=' . $id . '"><i class="fa fa-plus-square"></i> Add record</a> |
                <a href="' . container()->getUrlsite() . 'tma.php?action=import&amp;table=' . $id . '"><i class="fa fa-mail-forward"></i> Import data</a> |
                <a href="' . container()->getUrlsite() . 'tma.php?action=emptyTable&amp;id=' . $id . '"><i class="fa fa-ban"></i> Empty the table</a> |
                <a href="' . container()->getUrlsite() . 'tma.php?action=removeTable&amp;id=' . $id . '"><i class="fa fa-trash-o"></i> Delete the table</a>
            </p>';
            return $html;
        }
        ...
        private function deleteRecordConfirm()
        {
            ...
            if (strlen($id) && strlen($table) && strlen($token)) {
                if ($token == sha1('deleteRecord' . $id . $table . date('dmY'))) {
                    $db     = container()->tbm(
                        with(container()->tbm('core_table'))->find($table)->getName(),
                        with(container()->tbm('core_table'))->find($table)->getNs()
                    );
                    $obj    = $db->find($id);
                    if (!empty($obj)) {
                        $obj->delete();
                        header('Location: ' . container()->getUrlsite() . 'tma.php?action=displayData&id=' . $table .'');
                    }
                } else {
        ...
        private function deleteStructureConfirm()
        {
            ...
            if (strlen($id) && strlen($token)) {
                if ($token == sha1('deleteStructure' . $id . date('dmY'))) {
                    $dbs = container()->tbm('core_structure');
                    $obj = $dbs->find($id);
                    if (!empty($obj)) {
                        $db = container()->tbm(
                            with(container()->tbm('core_table'))->find($obj->getTable())->getName(),
                            with(container()->tbm('core_table'))->find($obj->getTable())->getNs()
                        );
                        $data = $db->fetch()->exec();
                        foreach ($data as $row) {
                            $f = $obj->field();
                        ...
                        $obj->delete();
                        header('Location: ' . container()->getUrlsite() . 'tma.php?action=table&id=' . $obj->getTable() .'');
                    }
                } else {
                    return $this->error('Wrong token.');
                }
        ...
        private function deleteStructure()
        {
            ...
            if (strlen($id)) {
                ...
                if (!empty($obj)) {
                    ...
                    Click <a href="' . container()->getUrlsite() . 'tma.php?action=deleteStructureConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to confirm this deletion or <a href="' . container()->getUrlsite() . 'tma.php?action=table&amp;id=' . $obj->getTable() .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function deleteRecord()
        {
            ...
            if (strlen($id) && strlen($table)) {
                ...
                return '<p class="alert">
                Click <a href="' . container()->getUrlsite() . 'tma.php?action=deleteRecordConfirm&amp;table=' . $table .'&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to confirm this deletion or <a href="' . container()->getUrlsite() . 'tma.php?action=displayData&amp;id=' . $table .'">here</a> to go back.
                </p>';
            } else {
                return $this->error('Wrong request.');
            }
        ...
        private function emptyTable()
        {
            $id     = isAke($_REQUEST, 'id', null);
            if (strlen($id)) {
                $table  = with(container()->tbm('core_table'))->find($id);
                if (!empty($table)) {
                    $key = sha1('emptyTable' . $id . date('dmY'));
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . 'tma.php?action=emptyTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to empty the table &laquo; ' . with(container()->tbm('core_table'))->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . 'tma.php?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function removeTable()
        {
            ...
            if (strlen($id)) {
                ...
                if (!empty($table)) {
                    ...
                    return '<p class="alert">
                    Click <a href="' . container()->getUrlsite() . 'tma.php?action=removeTableConfirm&amp;id=' . $id .'&amp;token=' . $key .'">here</a> to drop the table &laquo; ' . with(container()->tbm('core_table'))->find(request()->getId())->getName() . ' &raquo, or <a href="' . container()->getUrlsite() . 'tma.php?action=displayData&amp;id=' . $id .'">here</a> to go back.
                    </p>';
                } else {
                    return $this->error('Wrong request.');
                }
        ...
        private function addTable()
        {
            ...
            $html .= '<p>
            <form action="' . container()->getUrlsite() . 'tma.php?action=addTable" method="post" id="addTable">
            <input type="text" required placeholder="Name of the database" name="ns" /><br>
            ...
            if (strlen($id) && strlen($token)) {
                ...
                if (!empty($table)) {
                    if ($token == sha1('removeTable' . $id . date('dmY'))) {
                        $db     = container()->tbm($table->getName(), $table->getNs());
                        $db->dropTable();
                        $dbs    = container()->tbm('core_structure');
                        ...
                        $table->delete();
                        header(
                            'Location: ' . container()->getUrlsite() . 'tma.php'
                        );
                    } else {
                        return $this->error('Wrong request.');
                    }
            ...
            if (strlen($id) && strlen($token)) {
                ...
                if (!empty($table)) {
                    if ($token == sha1('emptyTable' . $id . date('dmY'))) {
                        ...
                        $db->emptyTable();
                        header(
                            'Location: ' . container()->getUrlsite() . 'tma.php?action=displayData&id=' . $id
                        );
                    } else {
                        return $this->error('Wrong request.');
                    }
        ...
        {
            $t      = with(container()->tbm('core_table'))->find(request()->getId());
            if (empty($t)) {
                return $this->error("Please choose a valid table.");
            }
            $db     = container()->tbm($t->getName(), $t->getNs());
            ...
            $html .= '<form id="listForm" action="' . container()->getUrlsite() . 'tma.php?action=displayData&id=' . request()->getId() . '" method="post">';
            ...
            foreach ($res as $row) {
                ...
                foreach ($fields as $k => $field) {
                    ...
                    if (strstr($type, 'fk_')) {
                        ...
                        $fk = with(container()->tbm($fkTable, $tableFk->getNs()))->find($value);
                        if (!empty($fk)) {
                            $value = '<a class="fk" href="' . container()->getUrlsite() . 'tma.php?action=editRecord&amp;id=' . $fk->getId() . '&amp;table=' . $tableFk->getId() . '">' . $this->truncate($fk->string()) . '</a>';
                        }
                    }
                    if ($type == 'timestamp') {
                        $value = date('d/m/Y H:i:s', round($value / 1000));
                    }
                    if ($type == 'file') {
                ...
                $html .= '<td>
                    <a href="' . container()->getUrlsite() . 'tma.php?action=editRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Edit record" class="fa fa-edit"></i></a> |
                    <a href="' . container()->getUrlsite() . 'tma.php?action=duplicateRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Duplicate record" class="fa fa-copy"></i></a> |
                    <a href="' . container()->getUrlsite() . 'tma.php?action=deleteRecord&amp;id=' . $row->getId() . '&amp;table=' . request()->getId() . '"><i rel="tooltip" title="Delete record" class="fa fa-trash-o"></i></a>
                </td></tr>';
            }
        ...
        private function table()
        {
            ...
            foreach ($structures as $structure) {
                ...
                $html .= '<td>
                    <a href="' . container()->getUrlsite() . 'tma.php?action=editStructure&amp;id=' . $s->getId() . '"><i rel="tooltip" title="Edit structure" class="fa fa-edit"></i></a> |
                    <a href="' . container()->getUrlsite() . 'tma.php?action=deleteStructure&amp;id=' . $s->getId() . '"><i rel="tooltip" title="Delete structure" class="fa fa-trash-o"></i></a>
                </td>';
                $html .= '</tr>';
            }
            $html .= '</table>';
            $html .= '<p><a href="' . container()->getUrlsite() . 'tma.php?action=addStructure&amp;table=' . request()->getId() . '"><i class="fa fa-plus"></i> Add a field</a></p>';
            return $html;
        }
        ...
        private function inputForm($type, $field, $length, $required, $value, $structure)
        {
            ...
            if (strstr($type, 'blob') || strstr($type, 'binary')) {
                $type = 'object';
            }
            if (strstr($type, 'fk_')) {
                $table = repl('fk_', '', $type);
                $tableFk = $this->tableByValue($table);
                $db = container()->tbm($table, $tableFk->getNs());
                ...
                $select .= '</select>
                <a target="_plus" href="' . container()->getUrlsite() . 'tma.php?action=addRecord&amp;id=' . $tableFk->getId() . '">
            ...
            switch ($type) {
                case 'object':
                    return '<textarea ' . $require . ' maxlength="' . $length . '" class="input-medium" name="' . $field . '" id="' . $field . '">' . json_encode($value) . '</textarea>';
                    break;
                case 'set':
        ...
        {
            $dbTable    = container()->tbm('core_table');
            $dbs        = container()->tbm('core_structure');
            ...
            $html .= '<p class="first">
                <form action="' . container()->getUrlsite() . 'tma.php?action=import&amp;table=' . $table->getId() . '" method="post" id="import" enctype="multipart/form-data">
        ...
        private function importMap()
        {
            if (true === context()->isPost()) {
                ...
                Router::redirect(
                    container()->getUrlsite() . 'tma.php?action=displayData&id=' . $table->getId()
                );
            } else {
                return $this->error("An error occured. Please try again.");
            }
        ...
        private function map($structures, $table, $separator = '%%')
        {
            ...
            <form action="' . container()->getUrlsite() . 'tma.php?action=importMap&amp;table=' . $table->getId() . '" method="post" name="map" id="map"><input type="hidden" name="separator" id="separator" value="' . $separator . '" />
            ';
        ...
        private function editRecord()
        {
            ...
            $html .= '<form action="' . container()->getUrlsite() . 'tma.php?action=editRecord&amp;table=' . $table->getId() . '&amp;id=' . $record->getId() . '" method="post" id="editRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
        ...
        private function duplicateRecord()
        {
            ...
            $html .= '<form action="' . container()->getUrlsite() . 'tma.php?action=duplicateRecord&amp;table=' . $table->getId() . '&amp;id=' . $record->getId() . '" method="post" id="duplicateRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
            foreach ($structures as $structure) {
                $field      = $structure->field()->getName();
        ...
        private function addRecord()
        {
            ...
            $structures = $dbs->where('table = ' . $table->getId())->order('field')->exec(true);
            $html .= '<form action="' . container()->getUrlsite() . 'tma.php?action=addRecord&amp;id=' . $table->getId() . '" method="post" id="addRecord" enctype="multipart/form-data">
            <input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
            <table class="table">';
            foreach ($structures as $structure) {
                $field      = $structure->field()->getName();
        ...
        private function addStructure()
        {
            ...
            if (true === context()->isPost()) {
                ...
                Router::redirect(
                    container()->getUrlsite() . 'tma.php?action=table&id=' . request()->getTable()
                );
            }
            ...
            $html           .= '<p class="first">
            <form action="' . container()->getUrlsite() . 'tma.php?action=addStructure&amp;table=' . request()->getTable() . '" method="post" id="editStructure">
            <table class="table">
            <tr>
                <th>Name</th>
                <th>Type</th>
            ...
            <tr>
                <td><input required class="input-small" name="name" value="" id="name" type="text" /></td>
                <td>' . $selectTypes . '</td>
        ...
        private function editStructure()
        {
            ...
            if (true === context()->isPost()) {
                ...
                $structure->hydrate()->save();
                Router::redirect(
                    container()->getUrlsite() . 'tma.php?action=table&id=' . $structure->getTable()
                );
            }
            ...
            $html .= '<p class="first">
            <form action="' . container()->getUrlsite() . 'tma.php?action=editStructure&amp;id=' . request()->getId() . '" method="post" id="editStructure">
            <table class="table">
            <tr>
                <th>Type</th>
                <th>Length</th>
        ...
        {
            $dbt = container()->tbm('core_table');
            $t = $dbt->find($id);
            if (!empty($t)) {
                return container()->tbm($t->name, $t->ns);
            }
            return null;
        }
    ...
    set_time_limit(0);
    // $db = container()->nbm('test');
    // $i = 0;
    // $max = 0;
    // while ($i < $max) {
    //     $i++;
    //     $db->create()->setName(Utils::token())->setPrice(rand(100, 25000))->save();
    // }

    container()->tbm('core_auth')->create()->setLogin('gplusquellec')->setPassword(sha1('230266gp'))->save();

    $error = null;

    $dbAuth     = container()->tbm('core_auth');
    $session    = session('tma');
    ...
    if (true === $isAuth) {
        ...
        $rma        = new Tma();
        $content    = $rma->content($action, $tables);
    }
    ...
    <body>
        <div class="container">
            ...
            <h1 class="text-center">
                <span onclick="document.location.href = '<?php echo container()->getUrlsite(); ?>tma.php';" class="title">
                    <i class="fa fa-cogs fa-3x"></i>&nbsp;&nbsp;textMyAdmin
                </span>
            </h1>
            ...
            <div class="row">
                ...
                <div class="pull-right">
                    <a href="<?php echo container()->getUrlsite(); ?>tma.php?action=home">
                        <i class="fa fa-home"></i>
                    </a> |
                    <a href="<?php echo container()->getUrlsite(); ?>tma.php?action=logout">
                ...
                <div class="col-md-2">
                    ...
                    <div id="tables">
                        <ul class="listTable">
                            ...
                            <li>
                                <i rel="tooltip-t" title="Show data" onclick="document.location.href = '<?php echo container()->getUrlsite(); ?>tma.php?action=displayData&amp;id=<?php echo $table['id']; ?>';" class="linkTable link fa fa-list"></i>
                                <a class="linkTable" href="<?php echo container()->getUrlsite(); ?>tma.php?action=table&amp;id=<?php echo $table['id']; ?>">
                                    <?php echo $rma->truncate($table['name']); ?>
                                    (<?php echo $rma->truncate($table['ns']); ?>)
                                </a>
                            </li>
                            <?php endforeach; ?>
                        </ul>
